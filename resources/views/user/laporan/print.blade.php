...
<body>
    ...
    <div class="headline">
        LAPORAN SISWA PKL DI {{ Str::upper($user->internship->dudika ?? 'Belum Terdaftar') }}
    </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th rowspan="1">Hari Tanggal</th>
                    <th colspan="1">Judul</th>
                    <th rowspan="1">Deskripsi</th>
                    <th rowspan="1">Dokumentasi</th>
                </tr>
            </thead>
            <tbody>
                <!-- Baris data absensi akan digenerate oleh backend -->
                @forelse ($journal as $no => $data)
                    <tr>
                        <td>{{ $no + 1 }}</td>
                        <td>{{ \Carbon\Carbon::parse($data->tgl)->locale('id')->translatedFormat('l, d F Y') }}</td>
                        <td>{{ $data->judul }}</td>
                        <td> {{ $data->isi }}</td>
                        <td><img src="{{ asset('storage/user/journal/' . $safe . '/' . $data->foto) }}" alt=""
                                style="max-width: 60px; height: auto; object-fit: contain;">
                        </td>
                    @empty
